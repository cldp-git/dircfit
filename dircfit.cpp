...
#include "include/dirc_particle.h"
#include <TFile.h>
#include <TH2.h>
#include <TH1.h>
#include <TRandom3.h>
...
int main(int nargs, char* argv[])
{
	...
	if(inputfile){
		...
		num_runs=0;
		coverage_plot=false;
		std::ifstream f("../tracks_at_dirc_10k.csv");
		char  s[100];
		...
		std::vector<dirc_particle> particles;
		dirc_particle particle;
		unsigned int r = 0;

		//Check that the file is open
		if(!f.is_open()) printf("The file could not be opened.\n");
		else{ 
			//Read information into the temporary dirc_particle
			while(f >> particle.PID >> particle.BAR >> particle.x >> particle.y
					>> particle.t >> particle.theta >> particle.phi >> particle.E)
			{
				//Check that we don't have any NaN values
				if(particle.PID!=particle.PID||particle.BAR!=particle.BAR||particle.x!=particle.x||
						particle.y!=particle.y||particle.t!=particle.t||particle.theta!=particle.theta||
						particle.phi!=particle.phi||particle.E!=particle.E) 
				{
					continue;
				}
				//Fill the dirc_particle vector		
		...
		//primary and background particles
		dirc_particle p;
		dirc_particle bkg;
		for(unsigned int n=0;n<r;n++){
			p=particles[n];
			if(abs(p.PID)>7 && abs(p.PID)<13)//8 and 9 are pions, 11 and 12 are kaons
			{
				mc_tally++;
				//  printf("found PID=%i\n",p.PID);
				pion_mc_beta = dirc_model->get_beta(p.E,pimass);
				kaon_mc_beta = dirc_model->get_beta(p.E,kmass);
				...
				for (unsigned int i =0; i< r;i++){
					...
					if(abs(p.t-bkg.t)<time_window && i!=n){
						confounded_tally++; 
						if(abs(bkg.PID)==2 ||bkg.PID==3){
						...
						else if(abs(bkg.PID)==8||bkg.PID==9){
							pion_beta = dirc_model->get_beta(bkg.E,pimass);
						...
						else if(abs(bkg.PID)==11 || bkg.PID==12){
							kaon_beta = dirc_model->get_beta(bkg.E,kmass);
						...
						else if(abs(bkg.PID)==5 || bkg.PID==6){
							muon_beta = dirc_model->get_beta(bkg.E,mumass);
			...
			printf("\np.PID=%i loglikehood difference(pion-kaon)=%f\n",p.PID,llc-llf);
			if(abs(p.PID)==8||p.PID==9){
